...
export class ContactController {
  ...
  getReplies = asyncHandler(async (req: Request, res: Response): Promise<void> => {
    ...
    ResponseHelper.success(
      res,
      { replies: msg.replies, status: msg.status, read: msg.read },
      'Replies retrieved'
    );
  });
  ...
  replyToMessage = asyncHandler(async (req: Request, res: Response): Promise<void> => {
    ...
    // Upload each buffered attachment to Cloudinary and map to IAttachmentInput.
    // allSettled keeps a single failed upload from orphaning the rest or failing
    // the whole reply — failed attachments are logged and skipped.
    const files = (req.files as Express.Multer.File[]) ?? [];
    ...
    const attachments: IAttachmentInput[] = [];
    uploadResults.forEach((result, index) => {
      if (result.status === 'fulfilled') {
        attachments.push(result.value);
      } else {
        console.warn(
          `[Reply] Failed to upload attachment "${files[index]?.originalname}":`,
    ...
    try {
      ...
      // The reply was not stored (e.g. the email send failed), so remove the
      // freshly uploaded Cloudinary attachments — otherwise they would linger
      // as orphans that no record references.
      await deleteAssets(attachments.map((a) => a.storedName));
      throw error;
    }
  });
